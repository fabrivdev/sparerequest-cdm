...
const StockPDFUpload = ({ userId, onSuccess }: StockPDFUploadProps) => {
  ...
  return (
    <div className="border border-dashed border-border rounded-xl p-6 text-center space-y-3">
      ...
      <div>
        <h3 className="font-medium text-foreground">Cargar Stock desde PDF</h3>
        <p className="text-sm text-muted-foreground mt-1">
          Sube el reporte "Inventario de Stock" del sistema Datapar
        </p>
      </div>
      ...
      {isProcessing ? (
        ...
        <div className={`flex items-center justify-center gap-2 py-2 ${result.success ? 'text-green-600' : 'text-destructive'}`}>
          {result.success ? <CheckCircle className="w-5 h-5" /> : <AlertCircle className="w-5 h-5" />}
          <span className="text-sm font-medium">
            {result.success ? `${result.count} registros cargados exitosamente` : 'Error al procesar el archivo'}
          </span>
        </div>
      ) : null}
