...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'createDesarme') {
      ...
      const { brand, model, serial_number, client_name, branch, product_code, product_name, quantity, reason, is_urgent } = body;
      ...
      const { data, error } = await supabase
        ...
        .insert({
          created_by: userId,
          brand, model, serial_number, client_name, branch,
    ...
    if (action === 'getDesarmes') {
      ...
      if (error) {
        console.error('Error fetching desarmes:', error);
    ...
    if (action === 'getDesarmeDetail') {
      ...
      if (!desarmeId) {
        return new Response(JSON.stringify({ error: 'Falta desarmeId' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }

      const { data: desarme, error } = await supabase
        .from('desarmes')
        .select('*')
        .eq('id', desarmeId)
        .single();
      ...
      if (error || !desarme) {
        return new Response(JSON.stringify({ error: 'Desarme no encontrado' }), { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }

      const { data: logs } = await supabase
        .from('desarme_status_log')
        .select('*')
        .eq('desarme_id', desarmeId)
        .order('created_at', { ascending: true });
    ...
    if (action === 'quoteDesarme') {
      ...
      if (!desarmeId || quoted_value === undefined) {
        return new Response(JSON.stringify({ error: 'Faltan campos obligatorios' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }

      // Verify current status
      const { data: current } = await supabase.from('desarmes').select('status').eq('id', desarmeId).single();
      if (!current || current.status !== 'pendiente_cotizacion') {
        return new Response(JSON.stringify({ error: 'El desarme no está pendiente de cotización' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          quoted_value: parseFloat(quoted_value),
          quoted_deadline: quoted_deadline || null,
          quoted_shipping_method: quoted_shipping_method || null,
          quote_observations: quote_observations || null,
          quoted_by: userId,
      ...
      if (error) {
        console.error('Error quoting desarme:', error);
        return new Response(JSON.stringify({ error: 'Error al cotizar' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
    ...
    if (action === 'authorizeDesarme') {
      ...
      const { data: current } = await supabase.from('desarmes').select('status').eq('id', desarmeId).single();
      if (!current || current.status !== 'pendiente_autorizacion') {
        return new Response(JSON.stringify({ error: 'El desarme no está pendiente de autorización' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      ...
      if (error) {
        return new Response(JSON.stringify({ error: 'Error al autorizar' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
    ...
    if (action === 'rejectDesarme') {
      ...
      if (!desarmeId || !rejection_reason) {
        return new Response(JSON.stringify({ error: 'El motivo de rechazo es obligatorio' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }

      const { data: current } = await supabase.from('desarmes').select('status').eq('id', desarmeId).single();
      if (!current || !['pendiente_cotizacion', 'pendiente_autorizacion'].includes(current.status)) {
        return new Response(JSON.stringify({ error: 'El desarme no puede ser rechazado en su estado actual' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      ...
      if (error) {
        return new Response(JSON.stringify({ error: 'Error al rechazar' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
    ...
    if (action === 'generateOrder') {
      ...
      if (!desarme || desarme.status !== 'aprobado') {
        return new Response(JSON.stringify({ error: 'El desarme debe estar aprobado para generar pedido' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }

      // Create order linked to desarme
      ...
      const { data: order, error: orderError } = await supabase
        .from('orders')
        .insert({
          user_id: desarme.created_by,
          brand: desarme.brand,
          product_code: desarme.product_code,
          quantity: desarme.quantity,
          branch_destination: desarme.branch,
          shipping_method: desarme.quoted_shipping_method || 'aereo',
          observation,
          status: 'pending',
          order_destination: 'cliente',
        })
        .select()
        .single();
      ...
      if (orderError) {
        ...
        return new Response(JSON.stringify({ error: 'Error al generar pedido' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }

      // Link order to desarme
      ...
      if (updateError) {
        console.error('Error linking order to desarme:', updateError);
      }
    ...
    if (action === 'updateDesarmeStatus') {
      ...
      const { desarmeId, newStatus, observation } = body;
      if (!desarmeId || !newStatus) {
      ...
      if (!validStatuses.includes(newStatus)) {
        return new Response(JSON.stringify({ error: 'Estado inválido' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      ...
      if (newStatus === 'cerrado') updateData.closed_at = new Date().toISOString();
      ...
      if (error) {
        return new Response(JSON.stringify({ error: 'Error al actualizar estado' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
    ...
    // ===== GET TRACKING (machines desarmadas) =====
    if (action === 'getDesarmeTracking') {
      if (!(await checkPerm('seguimiento_desarme'))) {
        return new Response(JSON.stringify({ error: 'Sin permiso para seguimiento' }), { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }

      // Get all desarmes that are "active" (not closed/rejected)
      const { data, error } = await supabase
        .from('desarmes')
        .select('*')
      ...
      if (error) {
        return new Response(JSON.stringify({ error: 'Error al obtener seguimiento' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }

      // Enrich with linked order status
      ...
      if (orderIds.length > 0) {
        const { data: orders } = await supabase
          .from('orders')
          .select('id, status, order_number, estimated_delivery_date')
          .in('id', orderIds);
        orders?.forEach((o: any) => { orderStatusMap[o.id] = o; });
      }

      const enriched = data.map((d: any) => {
        const daysDisassembled = Math.floor((Date.now() - new Date(d.created_at).getTime()) / (1000 * 60 * 60 * 24));
        return {
          ...d,
          days_disassembled: daysDisassembled,
